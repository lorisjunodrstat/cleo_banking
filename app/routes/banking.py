...
##########################################
###### Importation de transactions via csv
##########################################
# ---- APIs ----
...
def nouvelle_ecriture_multiple():
        ...
        for i in range(len(dates)):
            try:
                ...
                data = {
                    ...
                    'montant_htva': Decimal(str(montant)), # Sera recalculé juste après
                    'description': descriptions[i] if i < len(descriptions) else '',
                    'id_contact': id_contact_ligne,
                    'reference': references[i] if i < len(references) else '',
                    'type_ecriture': types[i],
                    'tva_taux': Decimal(str(taux_tva_val)),
                    'utilisateur_id': current_user.id,
                    'statut': statut,
                    'devise': 'CHF',
                    'type_ecriture_comptable': 'principale'
    ...
    # GET request processing
        ...
        return render_template('comptabilite/nouvelle_ecriture_multiple.html',
            comptes=comptes,
            categories=categories,
            categories_avec_complementaires=categories_avec_complementaires, # Correction orthographe
            contacts=contacts,
            taux_disponibles=taux_disponibles,
            today=datetime.now().strftime('%Y-%m-%d'),
            statuts_disponibles=[
    ...
    if request.method == 'POST':
        try:
            ...
            if taux_soumis > 0:
                ...
                if float(taux_soumis) not in taux_valides_values:
                    flash(f"Erreur : Le taux de TVA {taux_soumis}% n'est pas valide pour l'année {annee_ecriture}.", "danger")
                    return redirect(url_for('banking.edit_ecriture', ecriture_id=ecriture_id, show_modal='liaison'))
...
def delete_groupe_ecritures(groupe_id):
    ...
    trésorerie + vente + TVA + frais d'une même opération.
    """
    try:
        # 🔧 Utiliser g.db_manager (pas g.db)
...
@login_required
def liste_taux_tva():
    taux = g.models.taux_tva_model.get_all()
    return render_template('tva/liste.html', taux=taux)

@bp.route('/tva/creer', methods=['POST'])
@login_required
def creer_taux_tva():
    data = {
    ...
    g.models.taux_tva_model.create(data)
    return redirect(url_for('banking.liste_taux_tva'))

@bp.route('/tva/<int:taux_id>/modifier', methods=['POST'])
@login_required
def modifier_taux_tva(taux_id: int):
    data = {
        'annee': int(request.form.get('annee')),
        'nom': request.form.get('nom'),
        'pays': request.form.get('pays'),
    ...
    g.models.taux_tva_model.update(taux_id, data)
    return redirect(url_for('banking.liste_taux_tva'))

@bp.route('/tva/<int:taux_id>/supprimer', methods=['POST'])
@login_required
def supprimer_taux_tva(taux_id: int):
...
@login_required
def api_taux_by_date():
    """Retourne les taux TVA au format JSON pour une date donnée"""
...
def supprimer_employe(employe_id, entreprise_id):
    try:
        ...
        entreprise = g.models.entreprise_model.get_by_id(current_user_id, entreprise_id)
        if not employe :
...
def planning_employe(employe_id):
    # Récupérer UN SEUL employé (évite l'erreur de type 'list')
    employe = g.models.employe_model.get_by_id(employe_id, current_user.id)
    if not employe:
        flash("Employé non trouvé.", "error")
        return redirect(url_for('banking.liste_employe'))
    ...
    # Récupérer les heures de cet employé pour la période
    heures = g.models.heure_model.get_h1d_h2f_for_period(
        user_id=current_user.id,
        employeur=employe.get('employeur', 'TBD'),
        id_contrat=employe.get('id_contrat', 1),
        annee=annee,
        mois=mois
    )
    ...
    return render_template(
        ...
        employe=employe,  # Objet unique (contient .id, .prenom, .nom)
        heures=heures,
        annee=annee,
        mois=mois
    )
...
def pos_edit_store(store_id):
    ...
    if request.method == 'POST':
        success = g.models.magasin_pos_model.update(store_id, current_user.id, {
            'nom_magasin': request.form.get('nom_magasin'),
            'adresse': request.form.get('adresse', ''),
            'ville': request.form.get('ville', ''),
            'code_postal': request.form.get('code_postal', ''),
...
@login_required
def pos_delete_store(store_id):
    success = g.models.magasin_pos_model.delete(store_id, current_user.id)
    if success:
...
def pos_pdv_list():
    ...
    for magasin in magasins:
        pdvs = g.models.pdv_pos_model.get_by_magasin(magasin['id'], user_id)
        for pdv in pdvs:
...
@login_required
def pos_delete_pdv(pos_id):
    success = g.models.pdv_pos_model.delete(pos_id, current_user.id)
    if success:
        ...
        if action == 'update_modifier':
            ...
            if nom:
                ...
                if taux_tva_form == '':
                    ...
                    taux_tva_option = safe_float(taux_tva_form)
...
@bp.route('/pos/modifier-option/<int:option_id>/edit', methods=['GET', 'POST'])
@login_required
def pos_edit_modifier_option(option_id):
    # Récupérer l'option existante
    option = g.models.option_modificateur_pos_model.get_by_id(option_id, current_user.id)
    if not option:
        flash('Option non trouvée.', 'error')
    ...
    if request.method == 'POST':
        ...
        if updated:
            flash('Option modifiée avec succès !', 'success')
            return redirect(url_for('banking.pos_modifier_options'))
        flash('Erreur lors de la modification.', 'error')
    
    return render_template('pos/edit_modifier_option.html', 
                         option=option,
                         modifiers=modifiers, 
                         articles=articles,
                         types_taxes_disponibles=types_taxes_disponibles)

@bp.route('/pos/modifier-option/<int:option_id>/delete', methods=['POST'])
@login_required
def pos_delete_modifier_option(option_id):
    success = g.models.option_modificateur_pos_model.delete(option_id, current_user.id)
    if success:
...
def pos_delete_article(article_id):
    success = g.models.article_pos_model.delete(article_id, current_user.id)
    if success:
...
@bp.route('/pos/caisse/sale', methods=['POST'])
@login_required
def pos_create_sale():
    data = request.get_json()
    if not data:
...
@login_required
def pos_close_work_period(period_id):
    if request.method == 'POST':
        montant_fin_reel = safe_float(request.form.get('montant_fin_reel', 0))
        success, msg = g.models.periode_travail_pos_model.fermer_caisse(
            period_id, current_user.id, {'montant_fin_reel': montant_fin_reel}
        )
        if success:
            flash(msg, 'success')
            return redirect(url_for('banking.pos_work_periods'))
...
@bp.route('/pos/article/<int:article_id>/taxes', methods=['GET', 'POST'])
@login_required
def pos_article_taxes_history(article_id):
    article = g.models.article_pos_model.get_by_id(article_id)
    if not article or article.get('utilisateur_id') != current_user.id:
        flash("Article non trouvé.", "error")
        return redirect(url_for('banking.pos_articles_list'))
...
@bp.route('/pos/import', methods=['GET', 'POST'])
@login_required
def pos_import_data():
    if request.method == 'POST':
...
def pos_import_items(df):
    ...
    for idx, row in df.iterrows():
        ...
        prix = 0.0 if is_variable else float(prix_val) if prix_val else 0.0
        cout = float(str(row.get('Cost', '0')).replace(',', '.')) if pd.notna(row.get('Cost')) else 0.0
        stock = int(float(str(row.get('Stock', '0')).replace(',', '.'))) if pd.notna(row.get('Stock')) else 0
        ...
        article_data = {
            ...
            'cout_unitaire': cout,
            'stock': stock,
            'stock_alerte': int(float(str(row.get('Low stock', '0')).replace(',', '.'))) if pd.notna(row.get('Low stock')) else 0,
            'is_variable_price': is_variable,
            'vendu_type': 'kg' if str(row.get('Sold by weight', 'N')).upper() == 'Y' else 'piece',
            'description': str(row.get('Description', '')) if pd.notna(row.get('Description')) else '',
            'code_barre': str(row.get('SKU', row.get('Barcode', ''))) if pd.notna(row.get('SKU')) else ''
        }
        
        # 3. Créer l'article via le modèle
        article_id = g.models.article_pos_model.create(current_user.id, article_data)
        
        if article_id:
            count += 1
            # TODO: Ajouter ici la logique pour créer les variantes et lier les modificateurs 
            # en utilisant g.models.variante_pos_model.create() etc., si nécessaire.
            
    return count

def pos_import_categories(df):
    count = 0
        ...
        if nom:
            ...
            if not any(m['nom_modificateur'] == nom for m in existing):
                g.models.modificateur_pos_model.create(current_user.id, {
                    ...
                    'prix_modificateur': float(str(row.get('Prix', '0')).replace(',', '.')) if pd.notna(row.get('Prix')) else 0.0,
                    'description': str(row.get('Description', '')) if pd.notna(row.get('Description')) else ''
                })
                count += 1
    return count
...
def pos_edit_payment_method(mode_id):
    ...
    comptes_charges = g.models.categorie_comptable_model.get_all_categories(user_id)
